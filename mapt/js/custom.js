...
function openChapter(hash, $checkLink) {
    ...
    resetVideoContainer();
    videoContainer = $chapter.find('video')[0];
    ...
    if (!$(videoContainer).hasClass('vjs-tech')){
...
jqueryDocument.ready(function() {
    ...
    if (hash && (hash !== '#home') && (hash !== '#')) {
        ...
        $('.video-container, .next-wrapper').hide();
    }   

    var firstHref = $('.book_navigation .sub-nav:first > a').attr('href');
    $('#home .col-sm-12').find('.btn').attr('href', firstHref);
